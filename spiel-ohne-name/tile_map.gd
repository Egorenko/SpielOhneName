...
func _ready() -> void:
	#save_structure(Vector4i(1, 0, 6, 7), "res://structures/House_1");
	#save_structure(Vector4i(8, 1, 7, 6), "res://structures/House_2");
	...
	for x in range((-Overworld_map_radius - 16.0) / float(ChunkSize) - 1, (Overworld_map_radius + 16.0) / float(ChunkSize)): 
		for y in range((-Overworld_map_radius - 16.0) / float(ChunkSize) - 1, (Overworld_map_radius + 16.0) / float(ChunkSize)):
			...
			#for xx in range(GrassTileDimensio[0], GrassTileDimensio[2]):
			#	for yy in range(GrassTileDimensio[1], GrassTileDimensio[3]):
